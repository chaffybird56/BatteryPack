...
def main(out_dir: Path) -> None:
    ...
    plot_sweep_heatmap(
        df, x="Ns", y="Np", value="viol_temp", out_dir=out_dir, title="Temp Violations (1=yes)", cmap="Reds"
    )
    plot_sweep_heatmap(
        df, x="peak_current_a", y="UA_w_per_k", value="peak_temp_k", out_dir=out_dir, title="Peak T vs Current & UA"
    )
