...
def velocity_to_current(
    ...
    P_battery_w[discharge_mask] = P_mechanical_w[discharge_mask] / (motor_efficiency * transmission_efficiency)
    P_battery_w[charge_mask] = P_mechanical_w[charge_mask] * regenerative_braking_efficiency
...
def generate_wltp_class3() -> DriveCycle:
    ...
    velocity_kmh = 40.0 + 30.0 * np.sin(2 * np.pi * t / 300.0) + 15.0 * np.sin(2 * np.pi * t / 600.0)
