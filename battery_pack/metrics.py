...
def calculate_comprehensive_metrics(
	...
	energy_discharge_wh = float(
		np.trapz(np.maximum(power_w, 0.0), time_s) / 3600.0
	)
	# Energy input (charge)
	energy_charge_wh = float(
		np.trapz(np.minimum(power_w, 0.0), time_s) / 3600.0
	)
	...
	round_trip_efficiency = (
		100.0 * energy_discharge_wh / abs(energy_charge_wh)
		if abs(energy_charge_wh) > 1e-6
		else 0.0
	)
...
def calculate_cycle_life_estimate(
	...
	current_capacity_percent = (
		100.0
		- (cycles_completed / max(1e-6, cycles_to_eol)) * capacity_fade_limit_percent
	)
