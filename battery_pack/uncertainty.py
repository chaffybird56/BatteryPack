...
class MonteCarloAnalysis:
    ...
    def run_single_sample(
        ...
        voltage_failure = (V_cell_min < self.uncertainty.V_min_cell_v) or (V_cell_min > self.uncertainty.V_max_cell_v)
    ...
    def run_analysis(
        ...
        results = Parallel(n_jobs=n_jobs)(
            delayed(self.run_single_sample)(cycle, sim_params, i) for i in range(self.uncertainty.n_samples)
        )
    ...
    for param_name, values in param_ranges.items():
        for val in values:
            ...
            if param_name == "R0_ohm":
                cell = CellParams(**{**base_cell.__dict__, "R0_ohm": val})
            elif param_name == "UA_w_per_k":
                thermal = ThermalParams(**{**base_thermal.__dict__, "UA_w_per_k": val})
