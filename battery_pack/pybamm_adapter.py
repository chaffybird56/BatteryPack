...
def try_generate_ocv_curve() -> Optional[PyBaMMOCV]:
	...
	try:
		...
		for s in soc:
			...
			U_p = float(param["Positive electrode OCP entropic change [V/K]"] * 0.0 + param["Positive electrode OCP [V]"])
			U_n = float(param["Negative electrode OCP entropic change [V/K]"] * 0.0 + param["Negative electrode OCP [V]"])
			ocv.append(U_p - U_n)
