...
class BatteryPackAdvanced:
	...
	def step(self, I_pack_a: float, dt_s: float) -> Dict[str, float]:
		...
		for i in range(self.Ns):
			...
			V_term, Vrc_next, soc_next = cell.step_voltage_states(I_cell, dt_s, self.V_rc1[i], self.thermal.T[i], self.soc[i])
			if self._ocv_lookup is not None:
		...
		for i in range(self.Ns):
			...
			cap_new, R0_new, R1_new = apply_aging(c.capacity_ah, c.R0_ohm, c.R1_ohm, dAh, self.thermal.T[i], self.adv.aging)
